...
export default function FlowCanvas({
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      // build a tentative edge list
      const tentativeEdges = addEdge({ ...params }, edgesNow);

      // propagate width/height from the source across the tentative graph
      ...
      // recompute formats downstream (so defaults align with new size)
      ...
      // recompute again so input->output copying & param overrides apply to all nodes
      const finalNodes = updatedNodes.map(recomputeIOFormatsFromParams);

      //  reread source/target from updated nodes and check compatibility
      ...
      if (!isValidAfter) {
        ...
        toast.success(
          `Propagation complete , filled downstream nodes with size values`,
        );
      }
    },
